...
- (BOOL) isKeyCode:(NSInteger)keyCode andFlagsTaken:(NSUInteger)flags error:(NSError **)error;
{
	...
	NSArray *globalHotKeys = [NSArray arrayWithArray:(NSArray *)(__bridge NSArray*)(tempArray)];
	...
	while (( globalHotKeyInfoDictionary = [globalHotKeysEnumerator nextObject] ))
	{
		...
		if ( (CFBooleanRef)CFBridgingRetain([globalHotKeyInfoDictionary objectForKey:(NSString *)kHISymbolicHotKeyEnabled]) != kCFBooleanTrue )
            continue;
